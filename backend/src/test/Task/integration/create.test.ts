...
describe('Unit tests for create tasks route', () => {
  beforeEach(() => {
    ...
    jest.spyOn(jwt, 'verify').mockReturnValue(tokenReturnMock as unknown as void);
  });
  ...
  it(' Should pass when correct data is provided', async () => {
    const res = await request(app).post('/task/create').set('authorization', tokenMock).send(taskInput)
